$(document).ready(function () {
    ...
    $(".sidebar-overlay").on("click", function () {
        ...
        $(this).removeClass('opened'); $wrapper.removeClass('slide-nav');
    ...
    if ($('.datanew').length > 0) {
        $('.datanew').DataTable({ "bFilter": true, "sDom": 'fBtlpi', 'pagingType': 'numbers', "ordering": true, "language": { search: ' ', sLengthMenu: '_MENU_', searchPlaceholder: "Rechercher...", info: "_START_ - _END_ de _TOTAL_ élements", }, initComplete: (settings, json) => { $('.dataTables_filter').appendTo('#tableSearch'); $('.dataTables_filter').appendTo('.search-input'); }, });
    }
    ...
    }, 500); if ($('.datetimepicker').length > 0) {
    ...
    if ($('.toggle-password').length > 0) {
        $(document).on('click', '.toggle-password', function () {
            $(this).toggleClass("fa-eye fa-eye-slash"); var input = $(".pass-input");
    ...
    if ($('.toggle-passworda').length > 0) {
        $(document).on('click', '.toggle-passworda', function () {
            $(this).toggleClass("fa-eye fa-eye-slash"); var input = $(".pass-inputs"); if (input.attr("type") == "password") { input.attr("type", "text"); }
    ...
    if ($('#timer-countercallback').length > 0) {
        $('#timer-countercallback').countdown({ from: 10, to: 0, timerEnd: function () { this.css({ 'text-decoration': 'line-through' }).animate({ 'opacity': .5 }, 500); } });
    }
    ...
    if ($slimScrolls.length > 0) {
        $slimScrolls.slimScroll({ height: 'auto', width: '100%', position: 'right', size: '7px', color: '#ccc', wheelStep: 10, touchScrollStep: 100 }); var wHeight = $(window).height() - 60; $slimScrolls.height(wHeight);
        $('.sidebar .slimScrollDiv').height(wHeight);
        $(window).resize(function () {
            var rHeight = $(window).height() - 60; $slimScrolls.height(rHeight);
    ...
    function init() {
        ...
        $('#sidebar-menu a').on('click', function (e) {
            ...
            if (!$(this).hasClass('subdrop')) {
                $('ul', $(this).parents('ul:first')).slideUp(250); $('a', $(this).parents('ul:first')).removeClass('subdrop');
    ...
    init(); $(document).on('mouseover', function (e) {
        ...
        if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            ...
            if (targ) {
                $('body').addClass('expand-menu'); $('.subdrop + ul').slideDown();
            } else {
        ...
        if ($('body').hasClass('mini-sidebar')) {
            ...
            $(this).removeClass('active'); $('.subdrop + ul').slideUp();
    ...
    $('.inc.button').click(function () {
        var $this = $(this), $input = $this.prev('input'), $parent = $input.closest('div'), newValue = parseInt($input.val()) + 1;
    ...
    $('.dec.button').click(function () {
        ...
        console.log($parent);
        $parent.find('.inc').addClass('a' + newValue); $input.val(newValue);
        newValue += newValue;
    });
    ...
    $('.counters').each(function () {
        ...
        $(document).on('click', '.toggle-password', function () {
            ...
            var input = $(".pass-input"); if (input.attr("type") == "text") {
                input.attr("type", "text");
            } else { input.attr("type", "password"); }
        });
    }
    ...
    if ($('.ligne').length > 0) {
        $('.ligne').each(function () {
            $(this).on('change', function () {
                if ($(this).closest('tr').hasClass('choisis')) {
                    $(this).closest('tr').removeClass('choisis');
                } else {
                    $(this).closest('tr').addClass('choisis');
                }
            });
        });
    }
